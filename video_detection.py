"""
Ref: https://github.com/TannerGilbert/MaskRCNN-Object-Detection-and-Segmentation
"""
...
print(sys.path.append(os.path.join(ROOT_DIR, "samples/coco/")))
import coco
...
def apply_mask(image, mask, color, alpha=0.5):
    ...
    for n, c in enumerate(color):
        image[:, :, n] = np.where(
            mask == 1, image[:, :, n] * (1 - alpha) + alpha * c, image[:, :, n]
        )
    return image
...
def display_instances(image, boxes, masks, class_ids, class_names, scores=None):
    ...
    for i, c in enumerate(colors):
        ...
        image = cv2.putText(
            image, caption, (x1, y1), cv2.FONT_HERSHEY_COMPLEX, 0.7, c, 2
        )
    return image
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="MaskRCNN Video Object Detection/Instance Segmentation"
    )
    parser.add_argument(
        "-v",
        "--video_path",
    ...
    parser.add_argument(
        "-s", "--show", default=True, action="store_false", help="Show output"
    )
